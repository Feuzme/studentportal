...
public class StudentPortalDao {
	...
	public StudentPortalEntity update(StudentPortalEntity StudentPortalEntity, Session session) throws ExceptionDAO {
		...
		try {
			...
			throw new ExceptionDAO("Problème dans l'update pour l'objet " + StudentPortalEntity.getClass().getName(),
					e);
		}
	...
	public void delete(StudentPortalEntity StudentPortalEntity, Session session) throws ExceptionDAO {
		Transaction tx = session.beginTransaction();
		try {
			session.delete(StudentPortalEntity);
			tx.commit();
		} catch (Exception e) {
			tx.rollback();
			throw new ExceptionDAO("Probleme d'insert pour l'objet " + StudentPortalEntity.getClass().getName(), e);
